// PUT /api/blog/approve?slug=xyz — Approves a pending submission (admin token required).
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return send(res, 200, {
      success: true,
      message: `Post "${slug}" has been approved and published.`,
      slug,
    });
  } catch {
    return send(res, 500, { error: "Failed to approve submission." });
  }
}
