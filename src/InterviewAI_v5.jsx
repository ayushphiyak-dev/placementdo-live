...
/* ── Navbar ── */
const NAV_LINK_STYLE = { background:"none", border:"none", cursor:"pointer", fontFamily:"'DM Sans',sans-serif", textDecoration:"none", display:"block" };
const NavA = ({ href, className, style, onClick, children }) => (
  <a href={href} className={className} style={{ ...NAV_LINK_STYLE, ...style }}
...
const Navbar = ({ view, onNav }) => {
  ...
  return (
    <>
      <motion.nav aria-label="Main navigation" initial={{ y:-64,opacity:0 }} animate={{ y:0,opacity:1 }} transition={{ duration:0.5 }}
        ...
        <a href="/" aria-label="PlacementDo home" style={{ textDecoration:"none" }} onClick={e=>{ e.preventDefault(); onNav("landing"); }}><Logo /></a>
        <div className="nav-links-desk" style={{ display:"flex", gap:4, alignItems:"center" }} role="menubar">
          {isLanding && (<>
            <NavA href="/#features-section" className="nav-link" onClick={()=>scrollTo("features-section")}>Features</NavA>
            <NavA href="/#pricing-section" className="nav-link" onClick={()=>scrollTo("pricing-section")}>Pricing</NavA>
            <NavA href="/blog" className="nav-link" onClick={()=>onNav("blog")}>Blog</NavA>
            <NavA href="/faq" className="nav-link" onClick={()=>onNav("faq")}>FAQ</NavA>
            <NavA href="/about" className="nav-link" onClick={()=>onNav("about")}>About</NavA>
            <div style={{ width:1, height:20, background:"var(--border)", margin:"0 6px" }} role="separator"/>
          </>)}
          {!isLanding && <NavA href="/" className="btn-ghost" onClick={()=>onNav("landing")}>← Home</NavA>}
          <NavA href="/signin" className="btn-secondary" style={{ fontSize:13, display:"flex", alignItems:"center", gap:6, padding:"8px 14px", borderRadius:10 }} onClick={()=>onNav("signin")}><LogIn size={14}/> Sign in</NavA>
          <NavA href="/dashboard" className="btn-primary" style={{ fontSize:13, display:"flex", alignItems:"center", gap:6, padding:"8px 14px", borderRadius:10 }} onClick={()=>onNav("dashboard")}>Get started <ArrowUpRight size={14}/></NavA>
        </div>
        <button className="hamburger btn-ghost" aria-label="Open menu" aria-expanded={mob} style={{ display:"none", padding:8 }} onClick={()=>setMob(o=>!o)}>
          {mob ? <X size={22}/> : <Menu size={22}/>}
        </button>
      </motion.nav>
      <AnimatePresence>
        {mob && (
          <motion.div role="menu" initial={{ opacity:0, y:-10 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-10 }} transition={{ duration:0.22 }}
            ...
            {isLanding && (<>
              <NavA href="/#features-section" className="nav-link" style={{ textAlign:"left" }} onClick={()=>scrollTo("features-section")}>Features</NavA>
              <NavA href="/#pricing-section" className="nav-link" style={{ textAlign:"left" }} onClick={()=>scrollTo("pricing-section")}>Pricing</NavA>
              <NavA href="/blog" className="nav-link" style={{ textAlign:"left" }} onClick={()=>{ onNav("blog"); setMob(false); }}>Blog</NavA>
              <NavA href="/faq" className="nav-link" style={{ textAlign:"left" }} onClick={()=>{ onNav("faq"); setMob(false); }}>FAQ</NavA>
              <NavA href="/about" className="nav-link" style={{ textAlign:"left" }} onClick={()=>{ onNav("about"); setMob(false); }}>About</NavA>
              <div style={{ height:1, background:"var(--border)", margin:"4px 0" }}/>
            </>)}
            {!isLanding && <NavA href="/" className="nav-link" style={{ textAlign:"left" }} onClick={()=>onNav("landing")}>← Home</NavA>}
            <NavA href="/signin" className="btn-secondary" style={{ justifyContent:"center", display:"flex", padding:"10px", borderRadius:10 }} onClick={()=>{ onNav("signin"); setMob(false); }}><LogIn size={14}/> Sign in</NavA>
            <NavA href="/dashboard" className="btn-primary" style={{ justifyContent:"center", display:"flex", padding:"10px", borderRadius:10 }} onClick={()=>{ onNav("dashboard"); setMob(false); }}>Get started <ArrowUpRight size={14}/></NavA>
          </motion.div>
        )}
      </AnimatePresence>
    </>
        ...
        <div className="feature-grid stagger">
          {[
            ...
            <motion.div key={title} initial={{ opacity:0,y:22 }} whileInView={{ opacity:1,y:0 }} transition={{ duration:0.42,delay:i*0.07 }} viewport={{ once:true }} className="feature-card">
              ...
              <h3 style={{ fontSize: 17, fontWeight: 700, color: "var(--slate)" }}>{title}</h3>
              <p style={{ fontSize: 14.5, color: "var(--slate-600)", lineHeight: 1.65 }}>{desc}</p>
            </motion.div>
          ))}
        </div>
    ...
    <footer style={{ background:"var(--slate)", borderTop:"1px solid rgba(255,255,255,.06)" }}>
      <div className="footer-grid">
        ...
        <div>
          ...
          <a href="/#features-section" className="footer-link" onClick={(e) => { e.preventDefault(); scrollTo("features-section"); }}>Features</a>
          <a href="/#pricing-section" className="footer-link" onClick={(e) => { e.preventDefault(); scrollTo("pricing-section"); }}>Pricing</a>
          <a href="/blog" className="footer-link" onClick={(e) => { e.preventDefault(); onNav("blog"); }}>Blog</a>
          <a href="/faq" className="footer-link" onClick={(e) => { e.preventDefault(); onNav("faq"); }}>FAQ</a>
        </div>
        ...
        <div>
          ...
          <a href="/about" className="footer-link" onClick={(e) => { e.preventDefault(); onNav("about"); }}>About</a>
          <a href="/contact" className="footer-link" onClick={(e) => { e.preventDefault(); onNav("contact"); }}>Contact</a>
          <a href="/privacy-policy" className="footer-link" onClick={(e) => { e.preventDefault(); onNav("privacy-policy"); }}>Privacy Policy</a>
          <a href="/terms-of-service" className="footer-link" onClick={(e) => { e.preventDefault(); onNav("terms-of-service"); }}>Terms of Service</a>
        </div>
      ...
      <div style={{ borderTop:"1px solid rgba(255,255,255,.07)", padding:"16px clamp(20px,5vw,60px)", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:10, maxWidth:1200, margin:"0 auto" }}>
        ...
        <div style={{ display:"flex", gap:16 }}>
          <a href="/privacy-policy" onClick={(e) => { e.preventDefault(); onNav("privacy-policy"); }} style={{ fontSize:12, color:"rgba(255,255,255,.25)", background:"none", border:"none", cursor:"pointer", transition:"color 0.15s", fontFamily:"'DM Sans',sans-serif", textDecoration:"none" }}
            onMouseEnter={e=>e.currentTarget.style.color="rgba(255,255,255,.6)"}
            onMouseLeave={e=>e.currentTarget.style.color="rgba(255,255,255,.25)"}>Privacy</a>
    ...
    <motion.div initial={{ opacity:0,y:18 }} animate={{ opacity:1,y:0 }} transition={{ duration:0.35 }} style={{ minHeight:"100vh", paddingTop:100, paddingBottom:80, background:"var(--slate-50)" }}>
      <div style={{ maxWidth:780, margin:"0 auto", padding:"0 20px" }}>
        <div style={{ textAlign:"center", marginBottom:52 }}>
          <h1 className="brig" style={{ fontSize:"clamp(28px,4vw,42px)", fontWeight:800, color:"var(--slate)", letterSpacing:"-0.03em", lineHeight:1.1 }}>Frequently Asked Questions</h1>
          <p style={{ fontSize:16, color:"var(--slate-500)", marginTop:14, maxWidth:500, margin:"14px auto 0", lineHeight:1.65 }}>Everything you need to know about PlacementDo, AI mock interviews, personas, pricing, and how it all works.</p>
        </div>
        <div className="card" style={{ overflow:"hidden" }}>
          {FAQS2.map(({ q, a }, i) => (
            <div key={i} className="faq-item">
              <button className="faq-btn" onClick={()=>setOpen(o=>o===i?null:i)} style={{ padding:"20px 24px" }} aria-expanded={open===i}>
                <span className="brig" style={{ fontSize:15.5, fontWeight:700, color:"var(--slate)", letterSpacing:"-0.01em", lineHeight:1.3 }}>{q}</span>
                <span className="faq-chevron" style={{ transform:open===i?"rotate(180deg)":"rotate(0deg)" }}><ChevronDown size={18}/></span>
              </button>
              <AnimatePresence initial={false}>
                {open === i && (
                  <motion.div key="ans" initial={{ height:0,opacity:0 }} animate={{ height:"auto",opacity:1 }} exit={{ height:0,opacity:0 }} transition={{ duration:0.28 }}>
                    <p style={{ fontSize:14.5, color:"var(--slate-600)", lineHeight:1.75, padding:"0 24px 22px", borderTop:"1px solid var(--border)" }}>{a}</p>
                  </motion.div>
                )}
              </AnimatePresence>
        ...
        <p style={{ textAlign:"center", marginTop:32, fontSize:14, color:"var(--slate-400)" }}>
          Still have questions? <a href="/contact" onClick={e=>{e.preventDefault();onNav("contact");}} style={{ color:"var(--teal)", fontWeight:600, textDecoration:"underline" }}>Contact our team</a>.
        </p>
      </div>
    </motion.div>
  );
...
/* --- Blog Hub --- */
const BLOG_POSTS = [
  { id:"blog-system-design", title:"How to Ace System Design Interviews in 2026", summary:"A complete guide to system design interviews at top tech companies covering scalability, database selection, caching strategies, CAP theorem, and how to communicate your architecture clearly under pressure.", tag:"System Design", readTime:"12 min", date:"Aug 2026", emoji:"🏗️" },
  { id:"blog-star-method",   title:"Mastering the STAR Method for Behavioral Interviews", summary:"How to structure winning behavioral answers using the Situation, Task, Action, Result framework. Includes full worked examples for leadership, conflict, failure, and ambiguity questions.", tag:"Behavioral", readTime:"10 min", date:"Aug 2026", emoji:"⭐" },
  { id:"blog-behavioral",    title:"The Complete Guide to Behavioral Interviews at Top Tech Companies", summary:"Core behavioral competencies that Google, Amazon, Meta, and Microsoft actually evaluate and how to prepare for each one with targeted, structured practice.", tag:"FAANG Prep", readTime:"14 min", date:"Aug 2026", emoji:"🎯" },
  { id:"blog-resume",        title:"Resume Tips for Software Engineers: How to Get Past the ATS", summary:"Actionable resume writing strategies for SWE roles: ATS keyword optimization, impact quantification, project framing, and what FAANG recruiters actually look for on your resume.", tag:"Resume", readTime:"9 min", date:"Aug 2026", emoji:"📄" },
];
...
const BlogHub = ({ onNav }) => (
  <motion.div initial={{ opacity:0,y:18 }} animate={{ opacity:1,y:0 }} transition={{ duration:0.35 }} style={{ minHeight:"100vh", paddingTop:100, paddingBottom:80, background:"var(--slate-50)" }}>
    <div style={{ maxWidth:900, margin:"0 auto", padding:"0 20px" }}>
      <div style={{ textAlign:"center", marginBottom:52 }}>
        <h1 className="brig" style={{ fontSize:"clamp(28px,4vw,48px)", fontWeight:800, color:"var(--slate)", letterSpacing:"-0.03em", lineHeight:1.1 }}>Interview Prep Blog</h1>
        <p style={{ fontSize:16, color:"var(--slate-500)", marginTop:14, maxWidth:560, margin:"14px auto 0", lineHeight:1.65 }}>Expert guides on acing technical and behavioral interviews at the world's most competitive tech companies.</p>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(380px,1fr))", gap:24 }}>
        {BLOG_POSTS.map(({ id, title, summary, tag, readTime, date, emoji }) => (
          <motion.article key={id} initial={{ opacity:0,y:20 }} whileInView={{ opacity:1,y:0 }} viewport={{ once:true }} transition={{ duration:0.4 }}
            className="card card-lift" style={{ padding:28, cursor:"pointer", display:"flex", flexDirection:"column", gap:12 }}
            onClick={()=>onNav(id)}>
            <div style={{ fontSize:36 }}>{emoji}</div>
            <Tag color="teal" size="xs">{tag}</Tag>
            <h2 className="brig" style={{ fontSize:18, fontWeight:700, color:"var(--slate)", letterSpacing:"-0.015em", lineHeight:1.3 }}>{title}</h2>
            <p style={{ fontSize:13.5, color:"var(--slate-500)", lineHeight:1.7, flex:1 }}>{summary}</p>
            <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, color:"var(--slate-400)" }}>
...
/* --- Blog Article: System Design --- */
const BlogSystemDesign = ({ onNav }) => (
  <BlogShell onNav={onNav}>
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:24, fontSize:15.5, color:"var(--slate-700)", lineHeight:1.8 }}>
      <p>System design interviews are the most feared round at FAANG and top-tier tech companies. Unlike algorithm questions, there is no single correct answer. The interviewer is evaluating how you think, how you communicate, and whether you can reason about trade-offs under ambiguity. This guide gives you a repeatable, proven framework to perform confidently in any system design interview in 2026 and beyond.</p>
      ...
      <p>Reserve the final 10 to 15 minutes for a deep dive on the highest-risk component of your design. Walk through concrete failure scenarios: what happens when the cache layer is unavailable? When the primary database goes down? How does the system degrade gracefully when downstream services fail? Discussing circuit breakers, retries with exponential backoff, and graceful degradation distinguishes senior-level candidates from those who can only design the happy path.</p>
      <div style={{ padding:20, background:"var(--teal-light)", borderRadius:12, border:"1px solid rgba(13,148,136,.2)", marginTop:8 }}>
        <h3 className="brig" style={{ fontSize:16, fontWeight:700, color:"var(--teal-dark)", marginBottom:8 }}>Practice System Design with PlacementDo</h3>
        <p style={{ fontSize:14, color:"var(--teal-dark)", lineHeight:1.65, margin:0 }}>PlacementDo AI personas run full system design sessions on any topic. The Stress-Tester persona probes your consistency vs availability trade-offs and challenges every decision, replicating the exact pressure of a real FAANG system design round.</p>
      </div>
    </div>
  </BlogShell>
);

/* --- Blog Article: STAR Method --- */
const BlogSTAR = ({ onNav }) => (
  <BlogShell onNav={onNav}>
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:24, fontSize:15.5, color:"var(--slate-700)", lineHeight:1.8 }}>
      ...
      <div style={{ padding:20, background:"var(--teal-light)", borderRadius:12, border:"1px solid rgba(13,148,136,.2)", marginTop:8 }}>
        <h3 className="brig" style={{ fontSize:16, fontWeight:700, color:"var(--teal-dark)", marginBottom:8 }}>Practice STAR with PlacementDo</h3>
        <p style={{ fontSize:14, color:"var(--teal-dark)", lineHeight:1.65, margin:0 }}>PlacementDo scores every behavioral answer against the STAR rubric in real time, flagging missing components, vague actions, and unquantified results as you speak. The Empath and Friendly Peer personas are specifically tuned for behavioral STAR practice.</p>
      </div>
    </div>
  </BlogShell>
);

/* --- Blog Article: Behavioral Interviews --- */
const BlogBehavioral = ({ onNav }) => (
  <BlogShell onNav={onNav}>
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:24, fontSize:15.5, color:"var(--slate-700)", lineHeight:1.8 }}>
      ...
      <p>Meta's behavioral interviews center on Move Fast, Be Bold, and Focus on Impact. They want to hear about launching quickly despite incomplete information, taking calculated risks that paid off, and driving measurable business impact at scale. Meta interviewers specifically penalize over-engineering and excessive caution when shipping. Stories demonstrating speed, comfort with ambiguity, and rapid iteration strongly resonate with Meta interviewers at all levels.</p>
      <h2 className="brig" style={{ fontSize:22, fontWeight:700, color:"var(--slate)" }}>Microsoft: Growth Mindset and Collaboration</h2>
      <p>Microsoft centers its behavioral evaluation on Satya Nadella's Growth Mindset, which is the belief that abilities develop through dedication, learning, and genuine collaboration. Microsoft interviewers specifically want to hear about times you acknowledged failure, sought feedback from others, helped colleagues grow, or fundamentally changed your approach based on new information. Intellectual humility and genuine curiosity are rewarded more at Microsoft than at any other major tech company.</p>
      <h2 className="brig" style={{ fontSize:22, fontWeight:700, color:"var(--slate)" }}>Universal Competencies to Prepare</h2>
      <p>Regardless of your target company, prepare distinct and quantified stories for each of these competency areas: Leadership and Influence covering leading without formal authority, Conflict Resolution covering navigating disagreement with peers or managers, Handling Failure covering what you learned and how you recovered, Navigating Ambiguity covering making confident decisions with incomplete data, Innovation covering introducing a new process or idea, Cross-Functional Collaboration, and Delivery Under Pressure covering meeting a critical deadline with limited resources.</p>
      <div style={{ padding:20, background:"var(--teal-light)", borderRadius:12, border:"1px solid rgba(13,148,136,.2)", marginTop:8 }}>
        <h3 className="brig" style={{ fontSize:16, fontWeight:700, color:"var(--teal-dark)", marginBottom:8 }}>Practice Company-Specific Behavioral Interviews with PlacementDo</h3>
        <p style={{ fontSize:14, color:"var(--teal-dark)", lineHeight:1.65, margin:0 }}>PlacementDo lets you target specific companies. Our Amazon LP mode probes you with LP-mapped follow-up questions. Our Google Googleyness mode tests intellectual humility and ambiguity tolerance. Our Meta mode evaluates speed-of-execution stories specifically.</p>
      </div>
    </div>
  </BlogShell>
);

/* --- Blog Article: Resume Tips --- */
const BlogResume = ({ onNav }) => (
  <BlogShell onNav={onNav}>
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:24, fontSize:15.5, color:"var(--slate-700)", lineHeight:1.8 }}>
      <p>Your resume is the most important document in your job search. Before you can demonstrate your system design skills or STAR method stories, your resume must survive two gauntlets: an Applicant Tracking System that filters by keywords, and a recruiter who spends approximately 7 seconds on initial review. This guide gives you a proven, structured approach to passing both filters and getting to the interview stage.</p>
      <h2 className="brig" style={{ fontSize:22, fontWeight:700, color:"var(--slate)" }}>1. Use a Clean, ATS-Friendly Format</h2>
      <p>Fancy two-column templates with graphics, tables, and decorative headers look impressive visually but are frequently unreadable by ATS software, which parses resumes as plain text. Multi-column layouts cause ATS parsers to read across columns row by row, creating nonsense strings that fail keyword matching entirely. Use a single-column format with standard section headers such as Experience, Education, Skills, and Projects. Use standard fonts like Calibri, Arial, or Garamond. Save as a docx file for ATS portal submissions and as a pdf for direct recruiter emails.</p>
      <h2 className="brig" style={{ fontSize:22, fontWeight:700, color:"var(--slate)" }}>2. Quantify Every Single Bullet Point</h2>
      <p>Both ATS systems and human reviewers respond strongly to numbers. Saying you improved API performance is weak. Saying you reduced average API response time from 800 milliseconds to 120 milliseconds using Redis caching, which cut infrastructure costs by 24,000 dollars annually, is strong. For every bullet in your experience section, ask yourself: how much, how many, by what percentage, and over what time frame? If you genuinely cannot quantify a contribution, reconsider whether it belongs on your resume at all.</p>
      <h2 className="brig" style={{ fontSize:22, fontWeight:700, color:"var(--slate)" }}>3. Mirror ATS Keywords from the Job Description</h2>
      <p>Most ATS systems score resumes by counting keyword matches against the specific job description. Before submitting any application, identify the technical skills, tools, and methodologies explicitly named in the job description. Ensure those exact terms appear in your resume. Mirror the exact language of the job description wherever it is truthful. If the job description says Kubernetes, do not rely on k8s as your only mention because synonyms and abbreviations frequently fail ATS keyword matching algorithms.</p>
      <h2 className="brig" style={{ fontSize:22, fontWeight:700, color:"var(--slate)" }}>4. Structure Experience Entries for Maximum Recruiter Impact</h2>
      <p>Each role should include your company, job title, location, and dates, followed by 3 to 5 achievement bullets beginning with strong past-tense action verbs such as Architected, Reduced, Led, Implemented, Launched, or Mentored. Your most impactful bullet always comes first since recruiters skim top-down on their initial 7-second scan. Avoid passive verbs like helped, assisted, or worked on because they signal low ownership and follower-level participation to experienced recruiters.</p>
      <h2 className="brig" style={{ fontSize:22, fontWeight:700, color:"var(--slate)" }}>5. What FAANG Recruiters Actually Evaluate</h2>
      <p>Top-tier tech recruiters specifically look for four qualities on your resume. Scope and scale refers to whether you worked on systems serving millions of users. Complexity refers to whether the problem was genuinely technically hard. Ownership refers to whether you drove the work autonomously or followed instructions. Measurable impact refers to whether you moved a real business or technical metric. A resume targeting Google SWE L5 needs to demonstrate large-scale distributed systems thinking, not just feature delivery work. Tailor your bullet emphasis to match the target role and level explicitly in every application.</p>
      <div style={{ padding:20, background:"var(--teal-light)", borderRadius:12, border:"1px solid rgba(13,148,136,.2)", marginTop:8 }}>
        <h3 className="brig" style={{ fontSize:16, fontWeight:700, color:"var(--teal-dark)", marginBottom:8 }}>Upload Your Resume to PlacementDo</h3>
        <p style={{ fontSize:14, color:"var(--teal-dark)", lineHeight:1.65, margin:0 }}>PlacementDo parses your CV before each session to generate interview questions based specifically on your experience, projects, and claimed technologies. This replicates exactly what your real interviewer will have done before they meet you in person.</p>
      </div>
    </div>
  </BlogShell>
);
  ...
  <motion.div initial={{ opacity:0,y:18 }} animate={{ opacity:1,y:0 }} transition={{ duration:0.35 }} style={{ minHeight:"100vh", paddingTop:100, paddingBottom:80, background:"var(--slate-50)" }}>
    <div style={{ maxWidth:800, margin:"0 auto", padding:"0 20px" }}>
      ...
      <div className="card" style={{ padding:32, display:"flex", flexDirection:"column", gap:20 }}>
        <div>
          <h2 className="brig" style={{ fontSize:20, fontWeight:700, color:"var(--slate)", marginBottom:10 }}>1. Data Collection</h2>
          <p style={{ fontSize:15, color:"var(--slate-600)", lineHeight:1.7 }}>We collect audio, text inputs, and CV documents you provide during mock interviews. This data is exclusively used to generate real-time AI feedback and comprehensive performance reports to improve your interview skills. We also collect basic account information (name, email) for billing and authentication purposes.</p>
        </div>
        
        <div>
          <h2 className="brig" style={{ fontSize:20, fontWeight:700, color:"var(--slate)", marginBottom:10 }}>2. Use of AI and Third-Party Services</h2>
          <p style={{ fontSize:15, color:"var(--slate-600)", lineHeight:1.7 }}>Your interview responses may be processed by our secure, enterprise-grade LLM partners (e.g., OpenAI, Anthropic) to provide coaching. We do not use your personal interview data to train public AI models. All audio is processed ephemerally and transcribed securely.</p>
        </div>
        
        <div>
          <h2 className="brig" style={{ fontSize:20, fontWeight:700, color:"var(--slate)", marginBottom:10 }}>3. Data Retention</h2>
          <p style={{ fontSize:15, color:"var(--slate-600)", lineHeight:1.7 }}>Interview reports and performance metrics are retained as long as your account is active, allowing you to track progress over time. You may request account deletion at any time, which will permanently scrub all associated CVs and transcripts from our active databases within 30 days.</p>
        </div>
    ...
    <div style={{ maxWidth:800, margin:"0 auto", padding:"0 20px" }}>
      ...
      <div className="card" style={{ padding:32, display:"flex", flexDirection:"column", gap:20 }}>
        <div>
          ...
          <p style={{ fontSize:15, color:"var(--slate-600)", lineHeight:1.7 }}>PlacementDo provides AI-driven interview simulation and coaching. Our service is designed to help you practice and improve. We do not guarantee job placement, interview opportunities, or specific career outcomes.</p>
        </div>
        
        <div>
          <h2 className="brig" style={{ fontSize:20, fontWeight:700, color:"var(--slate)", marginBottom:10 }}>2. User Responsibilities</h2>
        ...
        <div>
          <h2 className="brig" style={{ fontSize:20, fontWeight:700, color:"var(--slate)", marginBottom:10 }}>3. Subscriptions and Credits</h2>
          <p style={{ fontSize:15, color:"var(--slate-600)", lineHeight:1.7 }}>Paid plans (Pro, Elite) are billed on a recurring monthly or annual basis. Interview credits expire at the end of each billing cycle and do not roll over. Refunds are handled on a case-by-case basis subject to our 14-day refund policy for unused credits.</p>
        </div>
      </div>
    </div>
  </motion.div>
    ...
    <div style={{ maxWidth:800, margin:"0 auto", padding:"0 20px" }}>
      ...
      <div className="card" style={{ padding:32, display:"flex", flexDirection:"column", gap:20 }}>
        ...
        <p style={{ fontSize:16, color:"var(--slate-600)", lineHeight:1.8 }}>
          PlacementDo was founded with a single mission: to level the playing field in technical and behavioral interviewing. 
          For too long, candidates who couldn't afford expensive coaching or didn't have industry connections were at a severe disadvantage.
        </p>
        <p style={{ fontSize:16, color:"var(--slate-600)", lineHeight:1.8 }}>
          By leveraging state-of-the-art Large Language Models and real-time voice synthesis, we've created a platform that simulates the exact pressure, pacing, and rigor of top-tier tech interviews (like FAANG). 
          Our AI personas—ranging from the empathetic listener to the brutal stress-tester—ensure that you are ready for any dynamic on the big day.
        </p>
        <div style={{ padding:"20px", background:"var(--teal-light)", borderRadius:12, border:"1px solid rgba(13,148,136,.2)", marginTop:10 }}>
          <h3 className="brig" style={{ fontSize:18, fontWeight:700, color:"var(--teal-dark)", marginBottom:8 }}>Our Core Values</h3>
          <ul style={{ paddingLeft:20, color:"var(--teal-dark)", lineHeight:1.7 }}>
            <li style={{ marginBottom:6 }}><strong>Accessibility:</strong> High-quality coaching should be available to everyone, regardless of background.</li>
            <li style={{ marginBottom:6 }}><strong>Realism:</strong> Our simulations must accurately reflect the actual difficulty of top tech interviews.</li>
            <li><strong>Actionable Feedback:</strong> We don't just give scores; we provide line-by-line coaching on exactly how to improve.</li>
          </ul>
        </div>
      </div>
    </div>
...
const Contact = () => (
  <motion.div initial={{ opacity:0,y:18 }} animate={{ opacity:1,y:0 }} transition={{ duration:0.35 }} style={{ minHeight:"100vh", paddingTop:100, paddingBottom:80, background:"var(--slate-50)" }}>
    <div style={{ maxWidth:600, margin:"0 auto", padding:"0 20px" }}>
      ...
      <p style={{ fontSize:15, color:"var(--slate-500)", marginBottom:30 }}>Have a question, partnership inquiry, or need support? We'd love to hear from you.</p>
      
      <div className="card" style={{ padding:32 }}>
        <form onSubmit={e => { e.preventDefault(); alert("Thanks for contacting us. We will get back to you shortly."); }} style={{ display:"flex", flexDirection:"column", gap:16 }}>
          <div>
            <label style={{ display:"block", marginBottom:6, fontSize:13, fontWeight:600, color:"var(--slate)" }}>Name</label>
            <input required placeholder="Your name" style={{ width:"100%", padding:"10px 14px", borderRadius:8, border:"1px solid var(--border)", fontSize:14 }} />
          </div>
          <div>
            <label style={{ display:"block", marginBottom:6, fontSize:13, fontWeight:600, color:"var(--slate)" }}>Email</label>
            <input required type="email" placeholder="you@example.com" style={{ width:"100%", padding:"10px 14px", borderRadius:8, border:"1px solid var(--border)", fontSize:14 }} />
          </div>
          <div>
            <label style={{ display:"block", marginBottom:6, fontSize:13, fontWeight:600, color:"var(--slate)" }}>Message</label>
            <textarea required rows={5} placeholder="How can we help?" style={{ width:"100%", padding:"10px 14px", borderRadius:8, border:"1px solid var(--border)", fontSize:14, resize:"vertical" }} />
          </div>
          <button type="submit" className="btn-primary" style={{ justifyContent:"center", padding:"12px", marginTop:10, fontSize:15 }}>Send Message</button>
        </form>
        
        <div style={{ marginTop:30, paddingTop:20, borderTop:"1px solid var(--border)", display:"flex", gap:20, flexWrap:"wrap" }}>
          <div>
            <span style={{ display:"block", fontSize:12, fontWeight:700, color:"var(--slate-400)", textTransform:"uppercase", letterSpacing:"0.05em", marginBottom:4 }}>Email Us</span>
            <span style={{ fontSize:14, color:"var(--slate-700)", fontWeight:500 }}>support@placementdo.app</span>
          </div>
          <div>
            <span style={{ display:"block", fontSize:12, fontWeight:700, color:"var(--slate-400)", textTransform:"uppercase", letterSpacing:"0.05em", marginBottom:4 }}>Headquarters</span>
            <span style={{ fontSize:14, color:"var(--slate-700)", fontWeight:500 }}>San Francisco, CA</span>
          </div>
        </div>
      </div>
    </div>
...
const NotFound = ({ onNav }) => (
  <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"var(--slate-50)", padding:20, textAlign:"center" }}>
    <div style={{ fontSize:64, marginBottom:16 }}>🗺️</div>
    <h1 className="brig" style={{ fontSize:32, fontWeight:800, color:"var(--slate)", marginBottom:12 }}>404 - Page Not Found</h1>
    <p style={{ fontSize:16, color:"var(--slate-500)", marginBottom:30, maxWidth:400 }}>The page you are looking for doesn't exist, has been moved, or is temporarily unavailable.</p>
    <button className="btn-primary" onClick={() => onNav("landing")} style={{ fontSize:15, padding:"12px 24px" }}>
      Return Home
    </button>
  </motion.div>
);
...
const SEO_META = {
  ...
  "dashboard":           { title:"Dashboard — Start a New AI Interview — PlacementDo", desc:"Configure and launch a new AI mock interview. Choose your target company, role level, and interviewer persona." },
  "reports":             { title:"My Interview Reports — PlacementDo", desc:"Review all your past AI mock interview sessions, scores, and performance trends over time." },
  "progress":            { title:"Progress Tracker — PlacementDo", desc:"Track your interview skill improvements over time across all dimensions: technical, behavioral, and communication." },
  "avatars":             { title:"AI Interviewer Personas — PlacementDo", desc:"Choose your AI interviewer: The Stress-Tester, The Empath, The Friendly Peer, The Stoic Veteran, The Devil's Advocate, and more." },
  "settings":            { title:"Account Settings — PlacementDo", desc:"Manage your PlacementDo account, subscription, language preferences, and notification settings." },
  "support":             { title:"Help & Support — PlacementDo", desc:"Find answers to common questions or contact the PlacementDo support team for assistance." },
  "privacy-policy":      { title:"Privacy Policy — PlacementDo", desc:"Read PlacementDo's privacy policy detailing how we handle your data, AI session recordings, and CV documents." },
  "terms-of-service":    { title:"Terms of Service — PlacementDo", desc:"Review the Terms of Service for PlacementDo's AI interview coaching platform, including usage terms and subscription details." },
  "about":               { title:"About PlacementDo — Our Mission to Democratize Interview Prep", desc:"Learn about the team and mission behind PlacementDo — making world-class interview coaching accessible to every candidate, regardless of background." },
  "contact":             { title:"Contact Us — PlacementDo", desc:"Get in touch with the PlacementDo team for support, partnerships, or press inquiries." },
  "faq":                 { title:"Frequently Asked Questions — PlacementDo AI Interview Coach", desc:"Find answers to the most common questions about PlacementDo: how AI interviews work, pricing, personas, languages, and more." },
  "blog":                { title:"Interview Prep Blog — Tips, Guides & Strategies — PlacementDo", desc:"Expert articles on acing technical and behavioral interviews: System Design, STAR Method, FAANG prep, resume tips, and more." },
  "blog-system-design":  { title:"How to Ace System Design Interviews in 2026 — PlacementDo Blog", desc:"A complete, step-by-step guide to system design interviews: scalability, CAP theorem, database choices, caching, and communication. Updated for 2026." },
  "blog-star-method":    { title:"Mastering the STAR Method for Behavioral Interviews — PlacementDo Blog", desc:"Learn how to structure compelling behavioral interview answers using the STAR method. Includes real examples for leadership, conflict, and failure questions." },
  "blog-behavioral":     { title:"The Complete Guide to Behavioral Interviews at Top Tech Companies — PlacementDo Blog", desc:"Everything you need to know about behavioral interviews at Google, Amazon, Meta, and Microsoft. Core competencies, sample questions, and scoring rubrics explained." },
  "blog-resume":         { title:"Resume Tips for Software Engineers: How to Get Past the ATS — PlacementDo Blog", desc:"Proven resume writing strategies for SWE roles: formatting, keyword optimization for ATS systems, quantifying impact, and what FAANG recruiters actually look for." },
  "404":                 { title:"404 — Page Not Found — PlacementDo", desc:"The page you are looking for does not exist. Return to PlacementDo's home page to explore our AI interview coaching platform." },
};
...
const PATH_MAP = {
  ...
  "faq":                 "faq",
  "blog":                "blog",
  "blog/system-design":  "blog-system-design",
  "blog/star-method":    "blog-star-method",
  "blog/behavioral-interviews": "blog-behavioral",
  "blog/resume-tips":    "blog-resume",
};

export default function App() {
  const [view, setView] = useState(() => {
    const path = window.location.pathname.substring(1);
    return PATH_MAP[path] ?? "404";
  });
  ...
  useEffect(() => {
    const onPopState = () => {
      const path = window.location.pathname.substring(1);
      setView(PATH_MAP[path] ?? "404");
    };
    window.addEventListener("popstate", onPopState);
    return () => window.removeEventListener("popstate", onPopState);
  }, []);

  // Dynamic SEO: title, description, canonical, robots — runs on every view change
  useEffect(() => {
    const meta = SEO_META[view] || { title:"PlacementDo", desc:"PlacementDo - AI Interview Coaching Platform." };
    ...
    // Robots — allow indexing on all public routes
    ...
    metaRobots.content = PUBLIC_ROUTES.includes(view) ? "index, follow" : "noindex, nofollow";

    // Canonical tag
    let canonical = document.querySelector('link[rel="canonical"]');
    if (!canonical) { canonical = document.createElement("link"); canonical.rel = "canonical"; document.head.appendChild(canonical); }
    const pathForView = Object.entries(PATH_MAP).find(([,v]) => v === view)?.[0] ?? "";
    canonical.href = `${BASE_URL}/${pathForView}`;

    // JSON-LD: FAQPage schema on landing & faq pages
    let ldScript = document.getElementById("ld-faq");
    if (view === "landing" || view === "faq") {
      if (!ldScript) { ldScript = document.createElement("script"); ldScript.id = "ld-faq"; ldScript.type = "application/ld+json"; document.head.appendChild(ldScript); }
  ...
  const go = v => {
    setView(v);
    const pathEntry = Object.entries(PATH_MAP).find(([,val]) => val === v);
    const urlPath = pathEntry ? (pathEntry[0] === "" ? "/" : `/${pathEntry[0]}`) : `/${v}`;
  ...
  const renderView = () => {
    ...
    if (view==="privacy-policy")   return <PrivacyPolicy />;
    if (view==="terms-of-service") return <TermsOfService />;
    if (view==="about")            return <About />;
    if (view==="contact")          return <Contact />;
    if (view==="faq")              return <FAQPage onNav={go}/>;
    if (view==="blog")             return <BlogHub onNav={go}/>;
    if (view==="blog-system-design") return <BlogSystemDesign onNav={go}/>;
    if (view==="blog-star-method")   return <BlogSTAR onNav={go}/>;
    if (view==="blog-behavioral")    return <BlogBehavioral onNav={go}/>;
    if (view==="blog-resume")        return <BlogResume onNav={go}/>;
    if (view==="404")              return <NotFound onNav={go}/>;
    if (isDash) {
      const inner =
